...
function [eigv,eigvec]=powerWithoutScalling(A,x0,tol)
  % A is the matrix
  % x0 is the iniatial guess for eigenvector
  % tol is the relative error
  % without scalling the formula is 
  % x_k=A*x_k-1 where k is the iteration number
  % default value of tol is 1%=0.01
  % default value of x0=[0.4;0.3;.07]
  ...
  % print some message
   ...
   N=1; % initial value of the iteration
    ...
    while true
      new_eigvec=A*old_eigvec; % matrix multiplication
