data=[18.802 21.232 44.567 35.62 42.38
...
42.445 17.107 43.300 38.32 28.82
...
% the text file constructed from the above data matrix
% this has been carried out to show you how to use process text file
writematrix(data,'crime.txt'); % store the matrix into a text file

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% copy code from here if you have the .txt file
% the above done just for demonstration
data=importdata('crime.txt');

% the household income is in column2
household_income=data(:,2);

% compute mean
Hmean=mean(household_income);

% median
Hmedian=median(household_income);

% standard deviation
Hstd=std(household_income);

% maximum
Hmax=max(household_income);

% minimum
Hmin=min(household_income);

% create a column vector to all the above statistics
statistics=[Hmean,Hmedian,Hstd,Hmax,Hmin]';

% create table
t=table(statistics,'VariableNames',{'HouseHold Statistics'});
