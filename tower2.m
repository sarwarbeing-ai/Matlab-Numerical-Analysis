clc;clear;close all;
format long g; % to have output in long format
...
alpha=1;
beta=1;
Niter=16; % no. of iterations ,you can change it if you want

for i=1:Niter
    F=alpha*(x.^-beta); % the function p
    DFalpha=x.^-beta; % derivative of p with respect to alpha
    
    % de vcbrivative of a^x with respect to x is ln(a)*a^x----formula
